        ...
        <section className="mt-2 text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {upcomingEvents.map((event, index) => (
              <div
                key={event.id}
                className="bg-white p-5 h-[50vh] rounded-lg shadow hover:shadow-lg transition-shadow duration-300 relative"
              >
                {index < 2 && (
                ...
                <img
                  src={
                    event.image ||
                    `https://via.placeholder.com/300x200?text=${event.name}`
                  }
                  alt={event.name}
                  className="w-full h-48 object-cover rounded-t-lg"
                />
                <h3 className="text-lg mt-10 text-black font-semibold ">
                  {event.name}
                </h3>
